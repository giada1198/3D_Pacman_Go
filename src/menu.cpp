...
void Menu::DrawGhost(int xdir, int ydir, int color, int gx, int gy, bool eatable) const {
    ...
    char curChar;
    int ghost_hei = 14; // hei and wid are hard coded
    int ghost_wid = 14;
    ...
    if (!eatable) {
        ...
        for (y = 0; y < ghost_hei; y++) {
            for (x = 0; x < ghost_wid; x++) {
                i = x;
                j = y;
                curChar = pattern[i + j*ghost_hei];
